package com.mattworzala.canary.server.world;
...
public class NoiseGenerator implements ChunkGenerator {
    ...
    private final JNoise jNoise2 = JNoise.newBuilder().perlin().setInterpolation(InterpolationType.LINEAR).setSeed(random.nextInt()).setFrequency(0.6).build();
    private final TreePopulator treeGen = new TreePopulator();
